...
class GeofenceApp {
    constructor() {
        ...
        // ตรวจสอบสถานะล็อกอิน 5 นาที (300,000 มิลลิวินาที) จาก Local Storage
        ...
        // 🔴 FIX: ตั้งค่า Light Mode (โดยใช้คลาส register-mode)
    ...
    checkAdminPasscode() {
        ...
        // ตรวจสอบรหัสผ่านใน ADMIN_USERS array ที่ดึงมาจาก Sheet
        ...
        if (authenticatedUser) {
            ...
            // บันทึก Timestamp และชื่อผู้ดูแลลง Local Storage
    ...
    init() {
        ...
        // ล้างประวัติ (History) ทันที เมื่อเป็นหน้า Studio
        if (this.studioName) {
    ...
    bindEvents() {
        // ผูก Event Listener สำหรับปุ่ม Retry ครั้งเดียวใน Init
        if (this.retryButton) {
    ...
    async fetchAnnouncementConfigFromSheet() {
        ...
        try {
            ...
            // ใช้ Full URL ที่ดึงมาจาก Sheet ทันที
    ...
    showGeofenceChecker() {
        // ถ้าเป็นหน้า Studio ให้แน่ใจว่าซ่อน Footer Countdown ไปด้วย
    ...
    startLoadCountdown(action) {
        ...
        if (this.modalLoaderText) {
             ...
             // 🔴 FIX: ใช้สีดำสำหรับ Light Mode
             this.modalLoaderText.style.color = '#1e293b';
        }
    ...
    startCloseButtonControl(action) {
        ...
        if (this.announcementControl.hideCloseBtn) {
            ...
            // ปิด Event Listener ชั่วคราวเมื่อนับถอยหลัง (ไม่ให้คลิกได้)
            ...
            this.countdownInterval = setInterval(() => {
                ...
                if (remaining < 0) {
                    ...
                    // เปิด Event Listener เมื่อนับเสร็จ
    ...
    geoSuccess(position) {
        ...
        if (distance <= this.target.dist) {
            ...
            // ปุ่ม Retry ต้องแสดงทันทีพร้อมหน้าผลลัพธ์
            this.retryButton.style.display = 'flex';
