...
let slideshowTimeout; // ใช้ Timeout เพื่อจัดการการเปลี่ยนช่วงเวลา
...
function fetchAndFilterImages() {
    ...
    images = allImages.filter(img => {
        ...
        // สร้าง Date object เปรียบเทียบเวลา
        const [hStart, mStart, sStart] = schedule.start_time.split(':').map(Number);
        const [hEnd, mEnd, sEnd] = schedule.end_time.split(':').map(Number);
        
        const nowTime = now.getHours() * 3600 + now.getMinutes() * 60 + now.getSeconds();
        const startTimeSec = hStart * 3600 + mStart * 60 + sStart;
        const endTimeSec = hEnd * 3600 + mEnd * 60 + sEnd;
...
function startSlideshow() {
    ...
    // ตรวจสอบ Index หากเกินจำนวนรูปภาพที่กรองได้ (อาจเกิดขึ้นเมื่อมีการอัปเดตข้อมูล)
...
// ตรวจสอบข้อมูลซ้ำทุกๆ 5 นาที (300000 ms) เพื่อรองรับการอัปเดตจากหน้า Admin ในเบราว์เซอร์อื่น
